...
angular.module('copayApp.controllers').controller('coinsriController',
  function($scope, $timeout,$state, $ionicModal, $log, storageService, coinsriService, ongoingProcess, platformInfo, externalLinkService, popupService, lodash) {

    var userId = localStorage.getItem('userId');

    if(userId)
    {
      $scope.message = "CoinSri's exchange service is available in the India, To buy or sell bitcoin Call on +91 7028909994";
    }
    else{
      $scope.message = "CoinSri's exchange service is available in the India, To buy or sell bitcoin please login";
    }
    ...
    $scope.openLogin = function()
    {
      ...
      if(userId)
      {
           window.open('https://coinsri.com/login','_system','location=yes');          
      }
      else
      {
